...
import { fetchRentals } from '../services/rentalApi'; // Giả sử fetchRentals là API gọi cho thuê
const { RangePicker } = DatePicker;


const RentalFilter = ({ setLoading, setRentals}) => {
    ...
    // Effect to call loadRentals when filters change
    useEffect(() => {
        loadRentals();
    ...
    const loadRentals = async () => {
        setLoading(true);
        try {
            const response = await fetchRentals(filters);  // Pass filters as a parameter to the API
        ...
        <div className="flex flex-wrap justify-between mb-4 gap-4">
            <div className="flex gap-4 flex-wrap w-full sm:w-auto">
                ...
                <div className="flex flex-col w-full sm:w-48">
                    <label className="mb-2 text-sm font-medium text-gray-700">Tìm tên cho thuê</label>
                    <Input
                        placeholder="Tìm tên cho thuê"
                        value={filters.name}
