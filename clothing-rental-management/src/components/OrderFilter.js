...
import { fetchOrders } from '../services/orderApi'; // Giả sử fetchOrders là API gọi giao dịch
const { RangePicker } = DatePicker;


const OrderFilter = ({ setLoading, setOrders}) => {
    ...
    // Effect to call loadOrders when filters change
    useEffect(() => {
        loadOrders();
    ...
    const loadOrders = async () => {
        setLoading(true);
        try {
            const response = await fetchOrders(filters);  // Pass filters as a parameter to the API
