...
export const fetchProducts = async (filters, itemType) => {
  try {
    let queryParams
    if (filters) {
      const { id, name, category, library, price, date } = filters
    ...
    const response = await api.get(
      `/products${queryParams ? `?${queryParams.toString()}` : ''}`
    )
...
export const addProduct = async (product, itemType) => {
  try {
    const response = await api.post(`/products?type=${itemType}`, product)
    return response.data
  } catch (error) {
...
export const updateProduct = async (id, product, itemType) => {
  try {
    const response = await api.put(`/products/${id}?type=${itemType}`, product)
    return response.data
  } catch (error) {
...
export const fetchContents = async (filters) => {
  try {
    ...
    const response = await api.get(
      `/contents${queryParams ? `?${queryParams.toString()}` : ''}`
    )
    return response.data
  } catch (error) {
    console.error('Lỗi khi lấy danh sách sản phẩm:', error)
...
export const fetchTopics = async (search) => {
  try {
    const response = await api.get(
      search ? `/topics?search=${search}` : '/topics'
    )
    return response.data
  } catch (error) {
