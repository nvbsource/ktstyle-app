    ...
    return new Promise((resolve, reject) => {
        db.query(query, [id], (err, results) => {
            if (err) {
                ...
                return reject(new Error(`Lỗi khi lấy dữ liệu từ bảng ${table} với ID ${id}`));
            }
            if (results.length === 0) {
...
module.exports = { beginTransaction, commitTransaction, rollbackTransaction, getAll, getById, query, where, filter, create, update, remove };
