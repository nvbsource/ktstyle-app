...
exports.getContents = (req, res) => {
  ...
  db.query(query, (err, results) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: 'Lỗi khi lấy bài viết và sản phẩm' })
    }
...
exports.createProductContent = (req, res) => {
  ...
  const query =
    'INSERT INTO Contents (product_id, content, image_urls, scheduled_at) VALUES (?, ?, ?, ?)'
  db.query(
    query,
    [product_id, content, JSON.stringify(image_urls), scheduled_at],
    (err, result) => {
      if (err) {
        console.log(err)
...
exports.updateProductContent = (req, res) => {
  ...
  const query =
    'UPDATE contents SET content = ?, image_urls = ?, scheduled_at = ?, status = ? WHERE id = ?'
  db.query(
    query,
    [content, JSON.stringify(image_urls), scheduled_at, status, id],
    (err, result) => {
      if (err) {
