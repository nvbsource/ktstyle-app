...
exports.updateColor = async (req, res) => {
    ...
    try {
        const existingColor = await dbRepository.where('Colors', { name });
        if (existingColor.data.length > 0 && existingColor.data[0].id !== parseInt(id)) {
            return errorResponse(res, 'Màu sắc với tên này đã tồn tại', 400);
        }

        const updatedColor = await dbRepository.update('Colors', id, { name, hex_code, rgb_code });
        successResponse(res, 'Màu sắc đã được cập nhật thành công', updatedColor.data);
    } catch (error) {
