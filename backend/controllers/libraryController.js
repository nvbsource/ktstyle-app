...
exports.getAllLibraries = async (req, res) => {
    try {
        ...
        console.error("Lỗi khi lấy danh sách thư viện", err);
...
exports.updateLibrary = async (req, res) => {
    ...
    try {
        ...
        console.error("Lỗi khi cập nhật thư viện", err);
...
exports.deleteLibrary = async (req, res) => {
    ...
    try {
        const productUses = await dbRepository.where('ProductLibrary', { library_id: id });
        if (productUses.data.length > 0) {
            return errorResponse(res, 'Thư viện này đang có sản phẩm không thể xoá', 400);
        }
