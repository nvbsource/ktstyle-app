...
exports.getProducts = async (req, res) => {
    ...
    try {
        ...
        let products = results.data.map(row => ({
            ...
            libraries: row.library_ids ? row.library_ids.split(',').map((id, index) => ({
                id: parseInt(id),
                name: row.library_names.split(',')[index]
            })) : []  // Kết hợp thư viện từ `GROUP_CONCAT`
        }));

        products = products.filter(product => {
            const matchLibrary = !library || product.libraries?.some(libraryObj => libraryObj.id == library);
            const matchCategory = !category || product.category?.id === parseInt(category);
...
exports.addProduct = async (req, res) => {
    const { name, description, notes, libraries, images, category_id, import_price, rental_price } = req.body;
    let errors = {};
    ...
    try {
        ...
        if (libraries && libraries.length > 0) {
            const productLibraries = libraries.map((libraryId) => [productId, libraryId]);
            // Sử dụng hàm query để thêm các danh mục vào bảng trung gian
            await dbRepository.query('INSERT INTO ProductLibrary (product_id, library_id) VALUES ?', [productLibraries]);
        }
...
exports.updateProduct = async (req, res) => {
    const { id } = req.params;
    const { name, description, notes, libraries, category_id, images, import_price, rental_price } = req.body;
