...
exports.updateTopic = (req, res) => {
    const { id } = req.params;
    const { name } = req.body;
    db.query('UPDATE topics SET ? WHERE id = ?', [{ name }, id], (err, result) => {
        if (err) {
...
exports.deleteTopic = (req, res) => {
    ...
    db.query('SELECT COUNT(*) AS count FROM product_contents WHERE topic_id = ?', [id], (err, results) => {
        if (err) {
        ...
        if (results[0].count > 0) {
            return errorResponse(res, "Không thể xóa chủ đề vì có nội dung liên kết", 400);
        }
